...
func yyNewParser(w http.ResponseWriter, r *http.Request) yyParser {
	c := appengine.NewContext(r)
	c.Infof("yyNewParser()")
	thisYY := &yyParserImpl{}
	c.Infof("thisYY: %v", thisYY)
	//fmt.Fprintf(w, "%#v\n", thisYY)
	return thisYY
}
...
func yyParse(w http.ResponseWriter, r *http.Request, yylex yyLexer) int {
	c := appengine.NewContext(r)
	c.Infof("yyParse()")
	yyp := yyNewParser(w,r).Parse(yylex)
	c.Infof("yyp: %#v", yyp)
	//return yyNewParser(w,r).Parse(yylex)
	return yyp
}
